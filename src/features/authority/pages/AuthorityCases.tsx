...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  ...
  closed:   { label: 'Cerrado',    color: 'var(--color-text-secondary, #475569)', bg: 'rgba(107,114,128,0.08)', dot: '#9CA3AF' },
}
  ...
  useRealtimeCases({
    onEvent: (payload) => {
      ...
      const nextStatus = deriveWorkflowStatus({
        workflow_status: nextRow.workflow_status,
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--color-background, #f8fafc)', fontFamily: 'system-ui, sans-serif' }}>
      <style>{`
        ...
        .auth-textarea {
          ...
          font-size: 13px; font-family: system-ui, sans-serif; resize: none;
          transition: border-color 0.15s, box-shadow 0.15s;
        }
          ...
          <div className="auth-card section-in" style={{ padding: '28px 32px', background: 'linear-gradient(135deg, #fff 0%, #F8F9FF 100%)' }}>
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: 'var(--color-text-secondary, #475569)', maxWidth: 480 }}>
          ...
          <div className="auth-card section-in section-in-1" style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ position: 'relative', flex: 1, minWidth: 220, maxWidth: 340 }}>
                <Search size={13} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--color-text-secondary, #475569)', pointerEvents: 'none' }} />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--color-text-secondary, #475569)' }}>
                <Filter size={12} />
                <span style={{ fontSize: 10, fontFamily: 'monospace', letterSpacing: '0.2em', textTransform: 'uppercase' }}>Estado</span>
              </div>
            </div>
          ...
          <div className="auth-card section-in section-in-2" style={{ overflow: 'hidden' }}>
            {loading ? (
              <div style={{ padding: '64px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                ...
                <p style={{ fontSize: 11, fontFamily: 'monospace', color: 'var(--color-text-secondary, #475569)' }}>Cargando casos...</p>
              </div>
            ) : filteredCases.length === 0 ? (
              <div style={{ padding: '64px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                <p style={{ fontSize: 14, color: 'var(--color-text-secondary, #475569)', fontWeight: 500 }}>No se encontraron casos.</p>
                <p style={{ fontSize: 12, color: 'var(--color-text-secondary, #475569)' }}>Intenta cambiar los filtros de búsqueda.</p>
              </div>
            ) : (
                    ...
                    <tr style={{ background: 'linear-gradient(180deg, #F8F9FB 0%, rgba(248,249,251,0) 100%)' }}>
                      {['No. Caso', 'Persona', 'Zona', 'Fecha', 'Estado', 'Acciones'].map((h) => (
                        <th key={h} style={{
                          ...
                          fontWeight: 500, color: 'var(--color-text-secondary, #475569)',
                    ...
                    {filteredCases.map((item, idx) => {
                      ...
                      return (
                        <tr key={item.id} className="case-tr" style={{ animationDelay: `${idx * 18}ms`, position: 'relative' }}>
                          {/* Row accent line */}
                          <td style={{ padding: '14px 20px', borderBottom: '1px solid var(--color-border, #e2e8f0)', background: idx % 2 !== 0 ? '#FAFBFC' : '#fff', position: 'relative' }}>
                            ...
                            <span style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--color-primary, #3266db)', fontWeight: 500 }}>{item.numero_caso}</span>
                          </td>
                          <td style={{ padding: '14px 20px', borderBottom: '1px solid var(--color-border, #e2e8f0)', background: idx % 2 !== 0 ? '#FAFBFC' : '#fff' }}>
                            <button type="button" onClick={() => navigate(`/authority/cases/${item.id}`)} style={{
                              background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left',
                              fontSize: 13, fontFamily: 'system-ui, sans-serif', fontWeight: 600, color: 'var(--color-text-primary, #0f172a)',
                          ...
                          <td style={{ padding: '14px 20px', borderBottom: '1px solid var(--color-border, #e2e8f0)', background: idx % 2 !== 0 ? '#FAFBFC' : '#fff' }}>
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, color: 'var(--color-text-secondary, #475569)' }}>
                              <MapPin size={11} style={{ color: 'var(--color-text-secondary, #475569)', flexShrink: 0 }} />
                              {getLocation(item)}
                            </span>
                          </td>
                          <td style={{ padding: '14px 20px', borderBottom: '1px solid var(--color-border, #e2e8f0)', background: idx % 2 !== 0 ? '#FAFBFC' : '#fff' }}>
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, color: 'var(--color-text-secondary, #475569)' }}>
                              <Calendar size={11} style={{ color: 'var(--color-text-secondary, #475569)', flexShrink: 0 }} />
                              {getDateLabel(item)}
                            </span>
                          </td>
                          <td style={{ padding: '14px 20px', borderBottom: '1px solid var(--color-border, #e2e8f0)', background: idx % 2 !== 0 ? '#FAFBFC' : '#fff' }}>
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setDeleteTarget(null) }}>
          <div className="modal-card" style={{ maxWidth: 420, borderLeft: '3px solid #DC2626' }}>
            <div style={{ padding: '28px 28px 24px' }}>
              ...
              <h2 style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 22, color: 'var(--color-text-primary, #0f172a)', fontWeight: 400, marginBottom: 8 }}>Confirmar eliminación</h2>
              <p style={{ fontSize: 13, color: 'var(--color-text-secondary, #475569)', lineHeight: 1.6, marginBottom: 24 }}>
                Estás por eliminar el caso{' '}
                <span style={{ fontFamily: 'monospace', color: 'var(--color-primary, #3266db)', fontWeight: 500 }}>{deleteTarget.numero_caso}</span>.
          ...
          <div className="modal-card" style={{ maxWidth: 580, borderLeft: '3px solid #0284C7' }}>
            <div style={{ padding: '28px 28px 24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 22 }}>
                ...
                <div>
                  <h2 style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 20, color: 'var(--color-text-primary, #0f172a)', fontWeight: 400 }}>Notas del caso</h2>
                  <p style={{ fontSize: 11, fontFamily: 'monospace', color: 'var(--color-primary, #3266db)', marginTop: 2 }}>{commentTarget.numero_caso}</p>
                </div>
              </div>

              {/* Comments list */}
              <div style={{ background: 'var(--color-background, #f8fafc)', border: '1px solid var(--color-border, #e2e8f0)', borderRadius: 8, padding: '12px 14px', maxHeight: 200, overflowY: 'auto', marginBottom: 14 }} className="auth-scroll">
                {(commentsByCaseId[commentTarget.id] ?? []).length === 0 ? (
                  <p style={{ fontSize: 12, color: 'var(--color-text-secondary, #475569)', fontFamily: 'monospace', padding: '8px 0' }}>Sin notas registradas para este caso.</p>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {(commentsByCaseId[commentTarget.id] ?? []).map((comment) => (
                      <div key={comment.id}>
                        ...
                        <p style={{ fontSize: 10, fontFamily: 'monospace', color: 'var(--color-text-secondary, #475569)', marginTop: 4, paddingLeft: 2 }}>
