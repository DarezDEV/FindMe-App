...
export default function AuthoritySightings() {
  ...
  const loadSightings = useCallback(async () => {
    ...
    try { setSightings(await getAuthoritySightings(300)) }
    catch (err) { setError(err instanceof Error ? err.message : 'No se pudieron cargar los avistamientos.') }
  ...
  const filteredSightings = useMemo(() => {
    ...
    return sightings.filter((item) => {
      const statusMatch = statusFilter === 'all' ? true : normalizeStatus(item.status) === statusFilter
      const searchMatch = term.length === 0 ? true : `${item.caseNumber ?? ''} ${item.missingPersonName ?? ''} ${item.location ?? ''} ${item.details}`.toLowerCase().includes(term)
