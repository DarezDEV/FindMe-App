...
const STATUS_CONFIG: Record<string, { label: string; dot: string; badge: string }> = {
  ...
  closed: { label: 'Cerrado', dot: 'bg-slate-400', badge: 'text-text-secondary bg-slate-400/10 border-slate-400/20' },
}
    ...
    <AdminSidebar>
      <div className="min-h-screen bg-background">
        <style>{`
          ...
          .input-dark { background: #f8fafc; border: 1px solid #e2e8f0; color: #0f172a; border-radius: 10px; padding: 10px 16px; width: 100%; outline: none; font-size: 13px; transition: border-color 0.15s; }
          .input-dark:focus { border-color: #fbbf24; }
          .input-dark::placeholder { color: #4a5568; }
          .textarea-dark { background: #f8fafc; border: 1px solid #e2e8f0; color: #0f172a; border-radius: 10px; padding: 12px 16px; width: 100%; outline: none; font-size: 13px; transition: border-color 0.15s; resize: none; font-family: inherit; }
          ...
          <div className="max-w-7xl mx-auto px-6 py-8 space-y-5">
            <div className="flex items-end justify-between">
              <div>
                ...
                <h1 className="text-3xl font-bold text-text-primary tracking-tight">Casos</h1>
                <p className="text-sm text-text-secondary mt-1">Gestión y seguimiento de todos los casos registrados en el sistema.</p>
              </div>
              <button
                type="button"
                onClick={loadCases}
                  ...
                  <button
                    ...
                    className={`status-pill inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                      statusFilter === value
                        ...
                        : 'bg-background text-text-secondary border-border hover:border-border hover:text-text-secondary'
                    }`}
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${cfg.dot}`} />
                    {cfg.label}
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden">
              {loading ? (
                <div className="p-16 flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-xs text-text-secondary font-mono">Cargando casos...</p>
                </div>
              ) : filteredCases.length === 0 ? (
                <div className="p-16 flex flex-col items-center justify-center gap-2">
                  <p className="text-text-secondary text-sm">No se encontraron casos.</p>
                  <p className="text-text-secondary text-xs">Intenta cambiar los filtros de búsqueda.</p>
                </div>
              ) : (
                    ...
                    <thead>
                      <tr className="border-b border-border">
                        {['Nº Caso', 'Persona', 'Zona', 'Fecha', 'Estado', 'Acciones'].map((h) => (
                          <th key={h} className="px-5 py-3.5 text-left text-[10px] font-mono font-medium text-text-secondary uppercase tracking-[0.15em]">
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {filteredCases.map((item, idx) => {
                        ...
                        return (
                          <tr
                            key={item.id}
                            className={`case-row border-b border-border last:border-b-0 ${idx % 2 === 0 ? '' : 'bg-background/50'}`}
                          >
                            ...
                            <td className="px-5 py-4">
                              <button
                                ...
                                className="text-left text-sm font-semibold text-text-primary hover:text-amber-300 transition-colors"
                              >
              ...
              {!loading && filteredCases.length > 0 && (
                <div className="px-5 py-3 border-t border-border flex items-center justify-between">
                  ...
                  <p className="text-xs font-mono text-text-secondary">{cases.length} en total</p>
                </div>
              )}
            </div>
          </div>
        ...
        {deleteTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-overlay">
            ...
            <div className="relative modal-card bg-card border border-border rounded-2xl p-7 w-full max-w-md shadow-2xl">
              <div className="w-12 h-12 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center mb-5">
                <Trash2 size={20} className="text-rose-400" />
              </div>
              <h2 className="text-lg font-bold text-text-primary mb-1">Confirmar eliminación</h2>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl bg-background border border-border text-text-secondary hover:text-text-primary text-sm font-medium transition-all"
                >
                  Cancelar
                </button>
                <button
        ...
        {commentTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-overlay">
            ...
            <div className="relative modal-card bg-card border border-border rounded-2xl p-7 w-full max-w-2xl shadow-2xl">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h2 className="text-base font-bold text-text-primary">Notas del caso</h2>
                  <p className="text-xs font-mono text-amber-400/70">{commentTarget.numero_caso}</p>
                </div>
              </div>

              <div className="rounded-xl border border-border bg-background p-4 space-y-2.5 max-h-52 overflow-y-auto mb-4">
                {(commentsByCaseId[commentTarget.id] ?? []).length === 0 ? (
                  <p className="text-xs text-text-secondary font-mono text-center py-3">Sin notas registradas para este caso.</p>
                ) : (
                  (commentsByCaseId[commentTarget.id] ?? []).map((comment) => (
                    <div key={comment.id} className="space-y-2">
                      ...
                      />
                      <p className="px-1 text-[11px] font-mono text-text-secondary">
              ...
              <div className="flex items-center gap-3">
                <button type="button" onClick={() => setCommentTarget(null)} className="flex-1 px-4 py-2.5 rounded-xl bg-background border border-border text-text-secondary hover:text-text-primary text-sm font-medium transition-all">
                  Cancelar
                </button>
                <button
                  type="button"
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-white text-sm font-bold disabled:opacity-40 transition-all"
                >
