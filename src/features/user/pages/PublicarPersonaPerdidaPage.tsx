...
export default function PublicarPersonaPerdidaPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const created = await publicarCaso(data)
      setCaseNumber(created.caseNumber)
      submit()
    } catch (error) {
      const message = error instanceof Error ? error.message : 'No se pudo publicar el caso.'
    ...
    <>
      ...
      <main className="min-h-screen bg-background py-8 px-4">
        <div className="max-w-2xl mx-auto">
          {submitted ? (
            <SuccessScreen data={data} caseNumber={caseNumber} onReset={handleReset} />
          ) : (
            <>
              <div className="mb-8">
                <Link
